...
import org.scalatest.{Inside, ShouldMatchers, FlatSpec}
import ra.analysis.rushing.data.{YardRange, AnalyzedRushRange, RushingDatum}
import ra.analysis.util.LoadUtils.getData

class EvaluateRushersSpec extends FlatSpec with Inside with ShouldMatchers {
  ...
  it should "produce metrics by range correctly for Spencer Ware" in {
    ...
    inside(s1) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s2) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s3) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s4) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s5) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s6) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s7) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s8) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s9) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
    ...
    inside(s10) { case AnalyzedRushRange(yardRange, avgRush, numRushes, numTds) =>
